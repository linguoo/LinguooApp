...
public class LinguooNoticiasActivity extends Activity implements OnInitListener,AccessibilityEventSource{
	...
	private void setTheView() {
		values=getResources().getStringArray(R.array.TitulosTxt);
		titulos = getResources().getStringArray(R.array.Titulos);
		noticias = getResources().getStringArray(R.array.Noticias);
			...
			if (inControl(lv,ev)){
				...
				if (position>=0 && position<lv.getCount()){
					if (actualpos!=position){
						Log.d(TAG,titulos[position]);
						humanit.sayIt(titulos[position]);
			...
			if (inControl(lv,ev)){
				...
				if (position>=0 && position<lv.getCount()){
					if ((nowclick-itemclicktime)<400 && actualpos==position){
						//launch(position);
						humanit.sayIt(noticias[position]);
